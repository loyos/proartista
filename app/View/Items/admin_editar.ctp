...
function genero1_change(){
	...
	$.ajax({
		...
		// $('#genero3').append($("<option></option>").attr("value", '').text('Selecciona un Género'));
		$.each(msg, function(i,a){
			if (a.Genero.nombre == '<?php echo $gen_dos?>') {
...
function genero2_change(){
	...
	$.ajax({
		...
		$('#genero3').append($("<option></option>").attr("value", '').text('Selecciona un Género'));
		$.each(msg, function(i,a){
			if (a.Genero.nombre == '<?php echo $gen_tres?>') {
...
function genero3_change(){
	...
	// if ()
	// alert(genero_dos);
	//if ((genero_dos == null || genero_dos == "") && ('<?php echo $gen_dos?>' == null || '<?php echo $gen_dos?>' == "")) {
	
		$.ajax({
			type: "POST",
			url: "/items/genero3_change.json",
			data: { genero_tres: genero_tres, genero_uno : genero_uno},
		dataType: "json"
		}).done(function( msg ) {
			// alert( "Data Saved: " + msg[1].Genero.nombre);
			$('#genero2 option').remove();
